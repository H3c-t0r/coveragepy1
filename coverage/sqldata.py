...
class CoverageData(AutoReprMixin):
    ...
    * **file tracer names**: the module names of the file tracer plugins that
    ...
    def __init__(self, basename=None, suffix=None, no_disk=False, warn=None, debug=None):
        ...
        if hasattr(os, "fork"):
            os.register_at_fork(after_in_child=functools.partial(self._at_fork, weakref.ref(self)))

    @staticmethod
    def _at_fork(self_ref):
        """A hook run in new child processes after a fork."""
        self = self_ref()
        if self is None:
            return
